...
- [ ] CPU 과다 점유의 정확한 원인 분석을 위한 종합 프로파일링 (진단 우선)
  - [ ] Rust 프로파일러(flamegraph, tracy 등)를 사용하여 기동 시 실시간 CPU 점유 병목 조사
  - [ ] 캡처 스레드 단독 구동 시 vs OCR 및 자켓 매칭 파이프라인 기동 시 vs egui 뷰포트 렌더링 시의 자원 사용량 대조
- [ ] 캡처 및 디텍션 제어 루프 최적화
