...
## 13. 결과창 OCR 정밀화 및 오버레이 반응성 개선 (백로그)

- [ ] 결과창 Rate OCR의 소수점 셋째 자리 `9` 반올림 오류 해결 (버림/절사 보정 추가)
- [ ] Score ROI 신설 및 스코어 OCR을 통한 판정율 역산(Rate = Score / 10,000) 및 크로스 검증(Cross-Validation) 적용
- [ ] OCR 3-pass 루프를 단일 패스(1-pass)로 롤백하여 CPU 연산 부하 축소
- [ ] Hysteresis 신뢰도가 0이 될 때 딜레이 없이 오버레이 창 즉시 숨김(0ms 반응성 확보)
- [ ] 결과창 진입 시 Hysteresis 감쇠 오동작을 차단하여 감지 끊김 Jitter 현상 제거
