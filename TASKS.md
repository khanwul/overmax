...
- [ ] 프리스타일 화면 이탈 시 오버레이 창 닫힘 반응 속도 개선 (연주 시작 시 즉시 닫히지 않고 한참 남아있는 현상 완화)
- [ ] 시스템 트레이 영역에서 설정창 등 보조 뷰포트를 호출할 때 런타임 갱신 주기 지연 현상 개선 (바로 뜨지 않고 대기하는 문제)
- [ ] 라이트모드 최초 기동/진입 시, 특정 기본 좌표(예: 오른쪽 중앙)에서 깜빡거린 뒤 목표 구석 위치로 이동하는 초기 렌더링 Jitter 현상 제거
- [ ] 오버레이 창에 마우스 호버 시 일시적으로 불투명해졌다가 다시 투명해지는 오작동 수정
