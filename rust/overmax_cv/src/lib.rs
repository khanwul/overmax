...
fn hashes_gray(data: &[u8], width: usize, height: usize) -> PyResult<(String, String, String)> {
    compute_hashes_gray(data, width, height).map_err(into_py_value_err)
}
...
fn image_features(
    ...
    compute_image_features(data, width, height, channels).map_err(into_py_value_err)
}

#[cfg(feature = "python")]
#[pyfunction]
fn thumbnail_bgra_32(data: &[u8], width: usize, height: usize) -> PyResult<Vec<u8>> {
    make_thumbnail_bgra_32(data, width, height).map_err(into_py_value_err)
}
...
fn ocr_preprocess_bgra(
    ...
    preprocess_ocr_bgra(data, width, height, force_invert).map_err(into_py_value_err)
}
