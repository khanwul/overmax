...
fn main() {
    ...
    paths.sort();
    println!(
        "Evaluating template matching on {} screenshots...",
        paths.len()
    );

    // cv_templates 매핑 구성
    let cv_templates: Vec<overmax_cv::CvTemplate> = DIGIT_TEMPLATES
        .iter()
    ...
    for path in paths {
        ...
        for &(x1, x2) in &segments {
            let char_bin = crop_binary_character(
                &binary,
                rate_img.width as usize,
                rate_img.height as usize,
                x1,
        ...
        let mut clean_matched: String = matched_str
            .chars()
            .filter(|c| c.is_ascii_digit() || *c == '.')
            .collect();
        let mut clean_expected: String = expected_str
            .chars()
            .filter(|c| c.is_ascii_digit() || *c == '.')
            .collect();
        ...
        if is_match {
            total_correct += 1;
            println!(
                "  [OK] {} -> Match SUCCESS. Matched: '{}', Expected: '{}'",
                filename, matched_str, expected_str
            );
        } else {
            println!(
                "  [FAIL] {} -> Match FAILED. Matched: '{}', Expected: '{}' (OCR value: {:.2}%)",
    ...
    println!(
        "Total Original Screenshots: {}",
        total_evaluated + total_skipped
    );
