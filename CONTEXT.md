...
- **배포 및 패키징 (Desktop Bridge / MSIX)**: Windows Desktop Bridge(Centennial) 기반 `AppxManifest.xml.template` 및 규격 비주얼 에셋(`packaging/msix/Assets/`)과 자동화 스크립트(`scripts/package-msix.ps1`) 구축. `runFullTrust` 권한, Windows SDK `MakeAppx`/`SignTool` 연동, Microsoft Store 등록 메타데이터 및 정책 대응 가이드(`docs/store/STORE_LISTING.md`, `docs/store/PRIVACY.md`) 완비 및 GitHub Actions CI 워크플로우(`ci.yml`) MSIX 아티팩트 빌드 연동.
