...
* [`RELEASE_NOTES_v0.4.1.md`](releasenotes/RELEASE_NOTES_v0.4.1.md): 실시간 로컬 IPC 이벤트 스트리밍(SSE) 및 원격 RPC 제어, Windows Store(MSIX) 패키징 파이프라인, Linux(Wayland/KDE/Niri) 오버레이 상태 분리, 장면 전환 DXGI 스왑체인 재생성 방지 및 플랫폼 간 워커/텔레메트리 일원화.
