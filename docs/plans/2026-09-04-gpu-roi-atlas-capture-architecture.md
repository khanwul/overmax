...
### Step 1: 컴파일 타임 1080p 아틀라스 레이아웃 & 트랜슬레이터 구현 [완료]
* [x] **`atlas_layout.rs` 정적 슬롯 테이블 베이킹**:
...
* [x] **`atlas_translator.rs` O(1) 점프 테이블 작성**:
...
### Step 6: 핑퐁 더블 버퍼링(Double Buffering) 구축 및 3단계 A/B/C 실측 기여도 분리 [완료]
* [x] **핑퐁 더블 버퍼링 Staging 텍스처 파이프라인 완성 (`dxgi.rs`)**:
  * 2개의 512×512 Staging 텍스처를 교대로 운용하고 새 프레임 복사 후 `context.Flush()`로 백그라운드 비동기 DMA 전송 개시.
  * CPU는 지난 틱에 전송이 완료된 이전 버퍼를 `Map`하여 **GPU 파이프라인 스톨 대기를 0ms로 소거**.
* [x] **실전 인게임 플레이 기반 A/B/C 3단계 기여도 완전 분리 검증**:
  * **[A] `main` (1080p 단일 버퍼)**: 캡처 평균 **4.50 ms** (P50: 4.55 ms, P95: 5.30 ms)
  * **[B] `fullframe-db` (1080p 더블 버퍼)**: 캡처 평균 **3.17 ms** (P50: 3.19 ms, P95: 3.69 ms)
  * **[C] `atlas-db` (512×512 아틀라스 더블 버퍼)**: 캡처 평균 **0.62 ms** (P50: 0.63 ms, P95: 0.80 ms)
  * **기여도 분리 판정 (The Definitive Attribution)**:
    * **[A ➔ B] 더블버퍼링의 순수 기여도**: **-1.33 ms 절감 (34.3% 기여)** — GPU 동기화 스톨 완전 소거
    * **[B ➔ C] 아틀라스의 순수 기여도**: **-2.55 ms 추가 절감 (65.7% 기여)** — 8.3MB ➔ 1.0MB 메모리/PCIe 대역폭 87.5% 절감
    * **최종 결론**: 더블버퍼링만으로는 3.17ms까지만 단축 가능하며, <1ms의 극저지연 및 고해상도(4K 33MB) 대역폭 방어를 위해서는 **아틀라스가 더블버퍼링보다 약 2배 더 결정적인 핵심 동력임**을 실측으로 완벽 입증함.
* [x] **리뷰어 지적사항 정밀 팩트체크 및 인과 규명**:
  * **정확도(12/19 vs 19/19)의 진실**: 캡처 지연 단축의 효과가 아니라, CV 글자 분할 알고리즘 버그(숫자 '1' 밑단 받침대 잘림)를 수정한 커밋 `8288bf1`의 순수 기여임이 확인됨.
  * **main vs feat (atlas off) 역전 현상의 진실**: 앱 시작 직후 1회성 초기화 스파이크(182ms vs 269ms)가 main에선 `Active`(선곡창)로, feat v2에선 `Ingame`으로 각각 다르게 분류되어 생긴 단순 웜업 분류 노이즈였음. 웜업 제외 시 main(선곡 4.99ms / 게임 4.40ms)과 feat v2(선곡 4.83ms / 게임 4.32ms)는 완전히 동일함.
  * **모든 벤치마크 원본 로그 보존**: `scratch/session_logs_backup/`에 SHA-256 해시와 함께 원본 세션 로그 5종 영구 보존 완료.
